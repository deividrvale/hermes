...
let input_format_eq f f' =
  match (f, f') with NONE, NONE -> true | ONIJN, ONIJN -> true | _, _ -> false

let format_to_string = function NONE -> "None" | ONIJN -> "ONijn"
...
let tt_to_string = function NONE -> "None" | TUPLE -> "Tuple"

let tt_eq x y =
  match (x, y) with NONE, NONE -> true | TUPLE, TUPLE -> true | _, _ -> false
...
let config =
  {
    ...
    technique = TUPLE;
    solver = Z3;
    tuples = { type_dim = 2; param_int = (0, 2); poly_deg = 2 };
  }
