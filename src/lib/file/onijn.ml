...
"The variable " ^ x ^ " is declared in the input file with the type " ^ (fakeTy_to_string fakeTy) ^ " which is an arrow type.
...
let register_signature (s : signature) =
  ...
  let fn_names = List.map fst s
    in List.iter
      (fun x -> let _ = func_symbolize
        (fun _ -> symbolize_err x ) x in ()) fn_names;
...
let register_enviroment (env : environment) =
  ...
  List.iter
      (fun x -> let _ = var_symbolize
        (fun _ -> symbolize_err x) x in ()) var_names;
  List.iter (fun (x,ty) -> var_set_sort (var_of_string x) (fakeTy_to_sort x ty)) env
