...
module SymInt () : SYMBOL = struct
  ...
  let rec mem_idx str lst idx =
    ...
    match lst with
    | [] -> idx
    | hd :: tl -> if String.equal str hd then idx else mem_idx str tl (idx + 1)

  let of_string_opt name =
    let idx = mem_idx name !token 0 in
    if idx >= !count then None else Some (!count - 1 - idx)
  ...
  let symbolize known name =
    (* known may raise exceptions *)
    match of_string_opt name with
    | None ->
        let n = !count in
        count := n + 1;
    ...
    | Some sym ->
        known sym;
        (* known provides a means of changing the control flow when the given name is already known *)
        sym
...
let typ_mk ins out =
  ...
  ( List.map symbolize ins,
    symbolize out (* sorts are not checked and registered on the fly *) )
...
let typ_is_sort (ty : typ) : bool =
  match typ_ins ty with [] -> true | _ -> false
...
let decl_set (decl : ('a * 'b) list ref) sym t = decl := (sym, t) :: !decl

let decl_get (decl : ('a * 'b) list ref) equal sym =
  !decl |> List.find (fun (s, _) -> equal s sym) |> snd
...
let sym_mk_opt name =
  match func_of_string_opt name with
  (* first check if name corresponds to any known function symbol *)
  | None -> Option.map (fun v -> V v) (var_of_string_opt name)
  | Some f -> Some (F f)
...
type term =
  (sym, typ) term_tree
  * typ (* a term is a pair consisting of its tree structure and its type *)
...
let rec term_mk_opt =
  ...
  | Sym name ->
      ...
      let ty =
        match sy with F f -> func_get_typ f | V v -> ([], var_get_sort v)
      in
      return (Sym sy, ty)
  | App ((tr1, _), (tr2, _)) -> (
...
let rec term_get_vars = function
  | Sym (F _), _ -> []
  | Sym (V v), _ -> v :: []
  | App (t1, t2), _ -> List.rev_append (term_get_vars t1) (term_get_vars t2)

let term_get_typ = snd

let rec term_get_head = function
  | Sym s, _ -> s
  | App (s1, _),_ -> term_get_head s1
...
let pp_typ_sep (f : formatter) () =
  pp_print_space f ();
  pp_print_string f "-->";
  pp_print_space f ()

let pp_print_sort f = pp_print_string f
...
let print_func func = pp_print_func std_formatter func
let pp_print_var (f : formatter) (x : var) = pp_print_string f (var_to_string x)
let print_var (x : var) = pp_print_var std_formatter x

let pp_print_symb (f : formatter) (s : sym) =
  match s with V x -> pp_print_var f x | F func -> pp_print_func f func

let print_symb (s : sym) = pp_print_symb std_formatter s
...
let rec pp_print_term' (f : formatter) (b : bool) (t : term) =
  ...
  | e ->
      if b then (
        (* an outer parenthesis on terms is not printed *)
        open_hbox ();
        print_app f e;
        close_box ())
      else (
        (* this case is only evaluated when printing applications on the rhs which doesnt occur in the root position*)
        open_hbox ();
        pp_print_string f "(";
        print_app f e;
        pp_print_string f ")";
...
let pp_print_term (f : formatter) (t : term) = pp_print_term' f true t
